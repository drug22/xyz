...
class Edit extends Component
{
    ...
    public function mount(Package $package)
    {
        ...
        $this->max_checklists = $package->max_checklists;
        $this->features = $package->features ?: [];
        $this->is_active = $package->is_active;
        $this->monthly_currency_prices = $package->monthly_currency_prices ?: [];
        $this->yearly_currency_prices = $package->yearly_currency_prices ?: [];
    }
        ...
        foreach ($supportedCurrencies as $currency) {
            if ($currency !== $defaultCurrency) {
                $this->monthly_currency_prices[$currency] = $service->convertPrice(
                    $this->monthly_price,
                    $defaultCurrency, // NU MAI E $this->base_currency!
                    $currency
                );
