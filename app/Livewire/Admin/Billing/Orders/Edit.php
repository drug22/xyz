...
class Edit extends Component
{
    ...
    protected $rules = [
        ...
        'status' => 'required|in:draft,published,pending,paid,failed,cancelled',
    ...
    public function mount(Order $order)
    {
        ...
        // Load existing data
        $this->assigned_to_user_id = $order->metadata['assigned_to_user_id'] ?? '';
        ...
        $this->vatValidated = $order->vat_number_validated ?? false;
        $this->vatValidationResult = $order->vat_validation_result ?? [];
    ...
    public function update()
    {
        ...
        Flux::toast(
            ...
            text: "Order #{$this->order->order_number} has been updated successfully.",
            variant: 'success'
        );

        return redirect()->route('admin.billing.orders.show', $this->order);
    }
    ...
    public function render()
    {
        ...
        $users = User::with('company')
            ...
            ->whereHas('roles', function($query) {
                $query->whereIn('name', ['owner', 'user']);
            })
            ->where(function($query) {
                $query->whereNull('company_id')
                    ->orWhere(function($subQuery) {
                        $subQuery->whereNotNull('company_id')
                            ->where('is_owner', true);
                    });
            })
            ->orderBy('name')
        ...
        return view('livewire.admin.billing.orders.edit', [
            'packages' => $packages,
            'countries' => $countries,
            'users' => $users,
